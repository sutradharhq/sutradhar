...
def main(argv: "list[str] | None" = None) -> int:
    ...
    if "-h" in argv or "--help" in argv:
        print(__doc__)
        return 0
    # An unrecognised flag is refused, never ignored: a `--selfcheck` that
    # is silently dropped proves only that the module imported (R17-2).
    print(
        f"[dead-route-lint] unknown argument(s): {' '.join(argv)}\n"
        f"dead_route_lint is a library; its CLI exists to run --selfcheck.",
        file=sys.stderr,
    )
    return 2
